                    ...
                    if isinstance(clue, int):  # deal with unknown operator
                        ...
                        if len(ar) == 2:
                            ...
                            self.add_program_line(
                                f":- number({r0}, {c0}, N0), number({r1}, {c1}, N1), (N0 - N1 * {clue}) * (N1 - N0 * {clue}) * (N0 - N1 - {clue}) * (N1 - N0 - {clue}) * (N0 + N1 - {clue}) * (N0 * N1 - {clue}) != 0."
                            )

                        if len(ar) > 2:
                            self.add_program_line(area_product_aggregate(_id=i, src_cells=ar))
                            self.add_program_line(
                                f":- S = #sum {{ N, R, C : area({i}, R, C), number(R, C, N) }}, area_product({i}, {len(ar) - 1}, P), (S - {clue}) * (P - {clue}) != 0."
                            )
                        ...
                        fail_false(operator in ("+", "*") or len(ar) == 2, f"Invalid clue at ({r}, {c}).")

                        if operator == "+":
                            self.add_program_line(f":- #sum {{ N, R, C : area({i}, R, C), number(R, C, N) }} != {num}.")

                        if operator == "-" and len(ar) == 2:
                            r0, c0 = ar[0]
                            r1, c1 = ar[1]
                            self.add_program_line(
                                f":- number({r0}, {c0}, N0), number({r1}, {c1}, N1), (N0 - N1 - {num}) * (N1 - N0 - {num}) != 0."
                            )
