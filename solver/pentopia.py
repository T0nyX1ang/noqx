...
def program(puzzle: Puzzle) -> str:
    ...
    solver.add_program_line(avoid_adjacent_omino(num=5, color="black"))

    solver.add_program_line(all_shapes("omino_5", color="black"))
    for i, o_shape in enumerate(OMINOES[5].values()):
        solver.add_program_line(general_shape("omino_5", i, o_shape, color="black", adj_type=4))
        solver.add_program_line(count_shape(("le", 1), name="omino_5", _id=i, color="black"))
    ...
    for (r, c, d, pos), symbol_name in puzzle.symbol.items():
        ...
        solver.add_program_line(opia_constraint(r, c, mask, max(puzzle.row, puzzle.col) + 1, color="black"))
