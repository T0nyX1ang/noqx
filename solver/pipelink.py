...
def adjacent_loop_intersect() -> str:
    ...
    adj += 'adj_loop_intersect(R, C, "H", R, C + 1, "H") :- grid(R, C), grid(R, C+1), line_io(R, C, "r").\n'
    adj += 'adj_loop_intersect(R, C, "V", R + 1, C, "V") :- grid(R, C), grid(R+1, C), line_io(R, C, "d").\n'
...
def intersect_loop(color: str = "white", path: bool = False) -> str:
    ...
    rule = "pass_by_loop(R, C) :- grid(R, C), #count { D: line_io(R, C, D) } = 2.\n"
