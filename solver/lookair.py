...
def square_size(color: str = "black") -> str:
    ...
    rule = f"square_size(R, C, N) :- top_left(R, C), MC = #min{{ C0: grid(R, C0 - 1), not {color}(R, C0), C0 > C }}, N = MC - C.\n"
...
def avoid_same_size_square_see(color: str = "black") -> str:
    ...
    rule += ":- top_left(R, C), left_square(R, C, MC), square_size(R, C, N), square_size(R, MC, N).\n"
    ...
    rule += ":- top_left(R, C), top_square(R, C, MR), square_size(R, C, N), square_size(MR, C, N).\n"
    rule += ":- top(R, C), top_square(R, C, MR), square_size(R, C, N), square_size(MR, C, N).\n"
    return rule
