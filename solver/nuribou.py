...
def noribou_strip_different(color: str = "black") -> str:
    ...
    rule = "nth(R, C, 1) :- top_left(R, C).\n"
    rule += "nth(R, C, N) :- top(R, C), nth(R, C - 1, N - 1).\n"
    rule += "nth(R, C, N) :- left(R, C), nth(R - 1, C, N - 1).\n"
    rule += f":- {color}(R, C), nth(R, C, N1), nth(R, C, N2), N1 < N2.\n"

    rule += "len_strip(R, C, 1) :- top_left(R, C), not top(R, C + 1), not left(R + 1, C).\n"
    rule += f"len_strip(R, C, N) :- top_left(R, C), top(R, C + 1), {color}(R, C + N - 1), not {color}(R, C + N), nth(R, C + N - 1, N).\n"
    rule += f"len_strip(R, C, N) :- top_left(R, C), left(R + 1, C), {color}(R + N - 1, C), not {color}(R + N, C), nth(R + N - 1, C, N).\n"
