...
class StatueParkSolver(Solver):
    ...
    def solve(self, puzzle: Puzzle) -> str:
        ...
        self.add_program_line(all_shapes("omino_x", color="gray"))

        shapeset = parse_shapeset(puzzle.param["shapeset"])
        for i, (o_shape, o_count) in enumerate(shapeset.items()):
            self.add_program_line(general_shape("omino_x", i, o_shape, color="gray", adj_type=4))
            self.add_program_line(count_shape(o_count, name="omino_x", _id=i, color="gray"))
