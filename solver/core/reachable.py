...
def grid_color_connected(
    ...
    initial_cell: Optional[Tuple[int, int]] = None,
...
def different_area_connected(tag: str):
    tag_decoded = tag.split("_")
    adj_type = tag_decoded[tag_decoded.index("adj") + 1]
    rule = f"{tag}(ID, R, C) :- clue(ID, R, C).\n"
    rule += f"{tag}(ID, R, C) :- {tag}(ID, R1, C1), clue(ID, _, _), grid(R, C), adj_{adj_type}(R, C, R1, C1).\n"
