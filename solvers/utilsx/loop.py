...
def loop_sign(color: str = "black") -> str:
    """Generate a rule for loop signs."""
    dirs = ["lu", "ld", "ru", "rd", "lr", "ud"]
    rule = ""
    for sign, (d1, d2) in zip(NON_DIRECTED, dirs):
        rule += f'loop_sign(R, C, "{sign}") :- {color}(R, C), grid_direction(R, C, "{d1}"), grid_direction(R, C, "{d2}").\n'
    return rule
