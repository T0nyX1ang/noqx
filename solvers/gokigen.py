...
directions = [Direction.TL, Direction.TR, Direction.BR, Direction.BL, Direction.SELF, Direction.NONE]
...
dir_to_offset = {
    Direction.TL: (-0.5, -0.5),
    Direction.TR: (-0.5, 0.5),
    Direction.BR: (0.5, 0.5),
    Direction.BL: (0.5, -0.5),
}

# String representations of the 4 commonly-used directions.
dir_to_str = {Direction.TL: "tl", Direction.TR: "tr", Direction.BR: "br", Direction.BL: "bl"}
...
def offset(d: Direction) -> Tuple[float, float]:
    """
...
def neighbors(r: int, c: int, direction: Direction, rows: int, cols: int) -> Tuple[List[Tuple[int]], List[Tuple[int]]]:
    """
    ...
    """
    needed, incompatible = [], []
    coord: Tuple[int, int] = tuple_sum((r, c), offset(direction))
    for d, delta in dir_to_offset.items():
        y, x = round_tuple(tuple_sum(coord, inverse(delta)))  # pylint: disable=unbalanced-tuple-unpacking
...
def every(lst: List) -> BoolVar:
    """
    If lst is empty, returns False; otherwise returns & of all BoolVars in lst.
    """
    if len(lst) > 0:
        return at_least(len(lst), lst)
    return BoolVar(False)


def none(lst: List) -> BoolVar:
    """
    If lst is empty, returns True; otherwise returns & of nots of all BoolVars in lst.
    """
    if len(lst) > 0:
        return at_most(0, lst)
    return BoolVar(True)
    ...
    for r in range(E.R):
        for c in range(E.C):
            ...
            for direction in dir_to_offset:
                ...
                require(every([grid[y][x][d] for (y, x, d) in needed]) | ~grid[r][c][direction])
                require(none([grid[y][x][d] for (y, x, d) in incompatible]) | ~grid[r][c][direction])
                ...
                if len(needed) > 0:
                    ...
                    dir_conn |= (parent[r][c] == direction) & conn[y][x] & (parent[y][x] != d)
                    ...
                    require(sum_bools(1, [parent[y][x] == d, parent[r][c] == direction]) | ~grid[r][c][direction])

            # Prove if this is a root or if the OR statement we defined above is ok.
            conn[r][c].prove_if((parent[r][c] == Direction.SELF) | dir_conn)
