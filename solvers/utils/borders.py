        ...
        for r in range(rows):
            for c in range(cols):
                for direction in DEFAULT_DIRECTIONS:
                    key = (r, c, direction)
                    self.__is_shaded[key], self.__connectivity[key], self.__chosen[key] = BoolVar(), Atom(), BoolVar()
                if r == rows - 1:
    ...
    def inside_loop(self, inside_cells):
        ...
        for r, c in inside_cells:
            ...
            for x in range(0, c + 1):
                even_num_edges_encountered = (self.is_shaded(r, x, Direction.LEFT) & ~even_num_edges_encountered) | (
