...
class RectangularGridShadingSolver:
        ...
        if known_root:
                ...
                for c in range(self.cols):
                    for y, x in connectivity_grid.get_neighbors(r, c):
                        connectivity_grid[r][c].prove_if(
                            ~var_in(self.grid[y][x], self.__shading_symbols) & connectivity_grid[y][x]
                        )
            ...
            for r in range(self.rows):
                for c in range(self.cols):
                    for y, x in connectivity_grid.get_neighbors(r, c):
                        connectivity_grid[r][c].prove_if(
                            chosen[r][c] | (~var_in(self.grid[y][x], self.__shading_symbols) & connectivity_grid[y][x])
                        )
                    require(connectivity_grid[r][c] | var_in(self.grid[r][c], self.__shading_symbols))
    ...
    def solutions(self, shaded_color="black"):
        ...
        return get_all_grid_solutions(
            ...
            format_function=lambda r, c: shaded_color
            if var_in(self.grid[r][c].value(), self.__shading_symbols)
            else "",
        )
