...
from .borders import Direction, get_border_coord_from_edge_id, get_edge_id
from .grids import RectangularGrid, is_valid_coord
...
class RectangularGridRegionSolver:
    ...
    def __init__(self, rows, cols, grid=None, given_regions=None, max_num_regions=None, region_symbol_sets=None):
        ...
        if given_regions == None:
            if max_num_regions == None or region_symbol_sets == None:
                raise ValueError(
                    "If a grid is being provided and regions are not being provided, max # regions and symbol sets must be provided"
                )
        ...
        for i in range(max_num_regions):
            require(
                at_most(
                    1,
                    [
                        (self.__region_id[r][c] == i) & (self.__parent[r][c] == ".")
                        for r in range(self.rows)
                        for c in range(self.cols)
                    ],
                )
    ...
    def set_region_size(self, max_region_size, clue_cells, min_region_size=0, clue_region_bijection=False):
        ...
        self.__region_size = [
            [IntVar(min_region_size, max_region_size) for c in range(self.cols)] for r in range(self.rows)
        ]
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                ...
                if r > 0:
                    ...
                    require(
                        (self.__region_size[r][c] == self.__region_size[r - 1][c]) | (self.__parent[r - 1][c] != "v")
                    )

                if r < self.rows - 1:
                    upstream_count += cond(self.__parent[r + 1][c] == "^", upstream[r + 1][c], 0)
                    require(
                        (self.__region_size[r][c] == self.__region_size[r + 1][c]) | (self.__parent[r + 1][c] != "^")
                    )

                if c > 0:
                    upstream_count += cond(self.__parent[r][c - 1] == ">", upstream[r][c - 1], 0)
                    require(
                        (self.__region_size[r][c] == self.__region_size[r][c - 1]) | (self.__parent[r][c - 1] != ">")
                    )
                ...
                if clue_region_bijection:
                    ...
                    if (r, c) in clue_cells:
                        require(
                            (self.__parent[r][c] == ".")
                            & (upstream[r][c] == clue_cells[(r, c)])
                            & self.__region_size[r][c]
                            == clue_cells[(r, c)]
                        )
                ...
                # A clue which is a root implies count
                    ...
                    require((self.__parent[r][c] != ".") | (upstream[r][c] == self.__region_size[r][c]))
                    if (r, c) in clue_cells:
                ...
                if (r, c) in region_root_to_id:
                    ...
                    if exact:
                        if region_symbol_set:
                            ...
                            require(~(var_in(self.grid[r][c], region_symbol_set) & (self.__parent[r][c] == ".")))
                        ...
                        require((self.__region_id[r][c] == (r * self.cols + c)) | (self.__parent[r][c] != "."))
    ...
    def set_num_neighbors_in_different_region(self, r, c, n):
        ...
        require(
            sum_bools(n - num_extra_edges, [self.__region_id[r][c] != self.__region_id[y][x] for (y, x) in neighbors])
        )
    ...
    def set_shaded_cells_in_region(self, clues, shading_symbols):
        ...
        """
        for r, c in clues:
            shaded_room_cells = [var_in(self.grid[y][x], shading_symbols) for (y, x) in self.get_region(r, c)]
    ...
    def set_unshaded_cells_in_region(self, clues, shading_symbols):
        ...
        """
        for r, c in clues:
            unshaded_room_cells = [~var_in(self.grid[y][x], shading_symbols) for (y, x) in self.get_region(r, c)]
    ...
    def get_shaded_cells_in_region(self, r, c, shading_symbols):
        ...
        """
        return sum_vars([var_in(self.grid[y][x], shading_symbols) for (y, x) in self.get_region(r, c)])
    ...
    def get_unshaded_cells_in_region(self, r, c, shading_symbols):
        ...
        """
        return IntVar(sum_vars([~var_in(self.grid[y][x], shading_symbols) for (y, x) in self.get_region(r, c)]))
    ...
    def get_neighbors_in_other_regions(self, r, c):
        ...
        """
        return list(filter(lambda neighbor: not self.is_in_same_region(r, c, *neighbor), self.grid.get_neighbors(r, c)))
    ...
    def solutions(self):
        ...
        def avoid_duplicate_solution():
            ...
            for r in range(self.rows):
                for c in range(self.cols - 1):
                    x &= (self.__region_id[r][c + 1] == self.__region_id[r][c]) == (
                        self.__region_id[r][c + 1].value() == self.__region_id[r][c].value()
                    )
            for r in range(self.rows - 1):
                for c in range(self.cols):
                    x &= (self.__region_id[r + 1][c] == self.__region_id[r][c]) == (
