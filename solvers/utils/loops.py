...
class RectangularGridLoopSolver:
    ...
    def __init__(self, rows, cols, directed=False, shading=False, min_num_loops=1, max_num_loops=1):
        """
    ...
    def loop(self, clue_cells, includes_clues=False, allow_blanks=True, transparent=False):
        """
            ...
            for c in range(self.cols):
                ...
                if 0 <= c - 1:
                    ...
                    if 0 <= r - 1:
                        if not self.__directed:
                            ...
                            require(
                                (
                                    var_in(self.grid[r][c - 1], RIGHT_CONNECTING)
                            ...
                            self.__loop_graph[r][c].prove_if(
                                self.__loop_start[r][c]
                                | (
                                    (self.grid[r][c] == "J")
                            ...
                            require(
                                (var_in(self.grid[r][c - 1], RIGHT_IN) & var_in(self.grid[r - 1][c], BOTTOM_OUT))
                                == (self.grid[r][c] == "J<")
                            )
                            ...
                            require(
                                (var_in(self.grid[r][c - 1], RIGHT_OUT) & var_in(self.grid[r - 1][c], BOTTOM_IN))
                                == (self.grid[r][c] == "J^")
                            )
                            # cell is connected to loop if it's a "loop start" or
                            # if its in neighbor is part of the loop
                            self.__loop_graph[r][c].prove_if(
                                self.__loop_start[r][c] | ((self.grid[r][c] == "J<") & (self.__loop_graph[r - 1][c]))
                            )
                            self.__loop_graph[r][c].prove_if(
                                self.__loop_start[r][c] | ((self.grid[r][c] == "J^") & (self.__loop_graph[r][c - 1]))
                            )
                    ...
                    if r + 1 < self.rows:
                        if not self.__directed:
                            ...
                            require(
                                (var_in(self.grid[r][c - 1], RIGHT_IN) & var_in(self.grid[r + 1][c], TOP_OUT))
                                == (self.grid[r][c] == "7<")
                            )
                            ...
                            require(
                                (var_in(self.grid[r][c - 1], RIGHT_OUT) & var_in(self.grid[r + 1][c], TOP_IN))
                                == (self.grid[r][c] == "7v")
                            )
                            # cell is connected to loop if it's a "loop start" or
                            # if its in neighbor is part of the loop
                            self.__loop_graph[r][c].prove_if(
                                self.__loop_start[r][c] | ((self.grid[r][c] == "7<") & (self.__loop_graph[r + 1][c]))
                            )
                            self.__loop_graph[r][c].prove_if(
                                self.__loop_start[r][c] | ((self.grid[r][c] == "7v") & (self.__loop_graph[r][c - 1]))
                            )
                            # cell has the same loop ID as its parent cell
                            if self.max_num_loops > 1:
                                require(
                    ...
                    if c + 1 < self.cols:
                        if not self.__directed:
                            ...
                            require(
                                (var_in(self.grid[r][c - 1], RIGHT_IN) & var_in(self.grid[r][c + 1], LEFT_OUT))
                                == (self.grid[r][c] == "-<")
                            )
                            ...
                            require(
                                (var_in(self.grid[r][c - 1], RIGHT_OUT) & var_in(self.grid[r][c + 1], LEFT_IN))
                                == (self.grid[r][c] == "->")
                            )
                            # cell is connected to loop if it's a "loop start" or
                            # if its in neighbor is part of the loop
                            self.__loop_graph[r][c].prove_if(
                                self.__loop_start[r][c] | ((self.grid[r][c] == "-<") & (self.__loop_graph[r][c + 1]))
                            )
                            self.__loop_graph[r][c].prove_if(
                                self.__loop_start[r][c] | ((self.grid[r][c] == "->") & (self.__loop_graph[r][c - 1]))
                            )
                ...
                if c + 1 < self.cols:
                    ...
                    if 0 <= r - 1:
                        if not self.__directed:
                            ...
                            #  - its right neighbor has flow left
                            require(
                                (var_in(self.grid[r - 1][c], BOTTOM_IN) & var_in(self.grid[r][c + 1], LEFT_OUT))
                                == (self.grid[r][c] == "L^")
                            )
                            ...
                            require(
                                (var_in(self.grid[r - 1][c], BOTTOM_OUT) & var_in(self.grid[r][c + 1], LEFT_IN))
                                == (self.grid[r][c] == "L>")
                            )
                            # cell is connected to loop if it's a "loop start" or
                            # if its in neighbor is part of the loop
                            self.__loop_graph[r][c].prove_if(
                                self.__loop_start[r][c] | ((self.grid[r][c] == "L^") & (self.__loop_graph[r][c + 1]))
                            )
                            self.__loop_graph[r][c].prove_if(
                                self.__loop_start[r][c] | ((self.grid[r][c] == "L>") & (self.__loop_graph[r - 1][c]))
                            )
                            # cell has the same loop ID as its parent cell
                            if self.max_num_loops > 1:
                                require(
                    ...
                    if r + 1 < self.rows:
                        if not self.__directed:
                            ...
                            #  - its bottom neighbor has flow up
                            require(
                                (var_in(self.grid[r][c + 1], LEFT_IN) & var_in(self.grid[r + 1][c], TOP_OUT))
                                == (self.grid[r][c] == "r>")
                            )
                            ...
                            #  - its bottom neighbor has flow in from the top
                            require(
                                (var_in(self.grid[r][c + 1], LEFT_OUT) & var_in(self.grid[r + 1][c], TOP_IN))
                                == (self.grid[r][c] == "rv")
                            )
                            # cell is connected to loop if it's a "loop start" or
                            # if its in neighbor is part of the loop
                            self.__loop_graph[r][c].prove_if(
                                self.__loop_start[r][c] | ((self.grid[r][c] == "r>") & (self.__loop_graph[r + 1][c]))
                            )
                            self.__loop_graph[r][c].prove_if(
                                self.__loop_start[r][c] | ((self.grid[r][c] == "rv") & (self.__loop_graph[r][c + 1]))
                            )
                            # cell has the same loop ID as its parent cell
                            if self.max_num_loops > 1:
                                require(
                ...
                if 0 < r and r + 1 < self.rows:
                    if not self.__directed:
                        ...
                        #  - its bottom neighbor has an upward-connecting edge
                        require(
                            (var_in(self.grid[r - 1][c], DOWN_CONNECTING) & var_in(self.grid[r + 1][c], UP_CONNECTING))
                            == (self.grid[r][c] == "1")
                        )
                        # cell is connected to loop if it's a "loop start" or
                        ...
                        self.__loop_graph[r][c].prove_if(
                            self.__loop_start[r][c]
                            | ((self.grid[r][c] == "1") & (self.__loop_graph[r - 1][c] | self.__loop_graph[r + 1][c]))
                        )
                        # cell has the same loop ID as the cells it's connected to
                        if self.max_num_loops > 1:
                            require(
                        ...
                        #  - its bottom neighbor has flow up
                        require(
                            (var_in(self.grid[r - 1][c], BOTTOM_IN) & var_in(self.grid[r + 1][c], TOP_OUT))
                            == (self.grid[r][c] == "1^")
                        )
                        ...
                        #  - its bottom neighbor has flow in from the top
                        require(
                            (var_in(self.grid[r - 1][c], BOTTOM_OUT) & var_in(self.grid[r + 1][c], TOP_IN))
                            == (self.grid[r][c] == "1v")
                        )
                        # cell is connected to loop if it's a "loop start" or
                        # if its in neighbor is part of the loop
                        self.__loop_graph[r][c].prove_if(
                            self.__loop_start[r][c] | ((self.grid[r][c] == "1^") & (self.__loop_graph[r + 1][c]))
                        )
                        self.__loop_graph[r][c].prove_if(
                            self.__loop_start[r][c] | ((self.grid[r][c] == "1v") & (self.__loop_graph[r - 1][c]))
                        )
                        # cell has the same loop ID as its parent cell
                        if self.max_num_loops > 1:
                            require(
        ...
        is_loop_start = [
            (self.__loop_start[r][c] & ~var_in(self.grid[r][c], ISOLATED))
            for c in range(self.cols)
            for r in range(self.rows)
        ]
        ...
        if self.max_num_loops > 1:
                ...
                for loop_id in range(self.max_num_loops):
                    ...
                    for r, c in region:
                        ...
                        if (r - 1, c) not in region:
                            num_outside_connections += cond(
                                var_in(self.grid[r][c], UP_CONNECTING + TOP_IN + TOP_OUT)
                                & (self.__loop_id[r][c] == loop_id),
                                1,
                                0,
                            )
                        # bottom neighbor
                        if (r + 1, c) not in region:
                            num_outside_connections += cond(
                                var_in(self.grid[r][c], DOWN_CONNECTING + BOTTOM_IN + BOTTOM_OUT)
                                & (self.__loop_id[r][c] == loop_id),
                                1,
                                0,
                            )
                        ...
                        if (r, c - 1) not in region:
                            num_outside_connections += cond(
                                var_in(self.grid[r][c], LEFT_CONNECTING + LEFT_IN + LEFT_OUT)
                                & (self.__loop_id[r][c] == loop_id),
                                1,
                                0,
                            )
                        ...
                        if (r, c + 1) not in region:
                            num_outside_connections += cond(
                                var_in(self.grid[r][c], RIGHT_CONNECTING + RIGHT_IN + RIGHT_OUT)
                                & (self.__loop_id[r][c] == loop_id),
                                1,
                                0,
                            )
                    # we should have either 0 outside connections (loop doesn't hit this region)
                    # or 2 outside connections (in/out)
            ...
            for region in regions:
                ...
                for r, c in region:
                    ...
                    if (r + 1, c) not in region:
                        num_outside_connections += cond(
                            var_in(self.grid[r][c], DOWN_CONNECTING + BOTTOM_IN + BOTTOM_OUT), 1, 0
                        )
                    # left neighbor
                    if (r, c - 1) not in region:
                        num_outside_connections += cond(
                            var_in(self.grid[r][c], LEFT_CONNECTING + LEFT_IN + LEFT_OUT), 1, 0
                        )
                    # right neighbor
                    if (r, c + 1) not in region:
                        num_outside_connections += cond(
                            var_in(self.grid[r][c], RIGHT_CONNECTING + RIGHT_IN + RIGHT_OUT), 1, 0
                        )
                # we should have either 0 outside connections (loop doesn't hit this region)
                # or 2 outside connections (in/out)
                require(num_outside_connections < 3)
