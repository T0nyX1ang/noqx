...
function get_id_arr(elt) {
  let arr = elt.id.split(",");
...
function get_text_width(txt, margin_px = 1) {
  ...
  if (get_text_width.ctx.font !== fontspec) get_text_width.ctx.font = fontspec;
...
function update_shift_click_css() {
  for (let cell of get_selected_cells()) cell.style.backgroundColor = "";
...
$(document).click(function (event) {
  handle_click(event, event.target);
});
...
function set_active(elt) {
  if (active_element != null) active_element.style.outline = "none";

  if (status == "solved") return;
  if (active_element != elt) {
    try {
      ELVES[active_element.id].handle_becoming_inactive();
    } catch {}
  }
  ...
  if (elt != null) {
    ...
    try {
      ELVES[elt.id].handle_becoming_active();
    } catch {}
  }
}
...
$(document).mousemove(function (event) {
  if (PUZZLE_TYPES[pt].properties.border && mouse_down) {
    ...
    if (key) {
      ...
      // use existing mode for this mousedown
      else ELVES[containing_cell_id].toggle_border(key, toggling_mode);
    }
  }
});
...
function get_captured_elt_id() {
  ...
  containing_cell_id = `${Math.floor(grid_mouse_y) * 2 + 1},${
    Math.floor(grid_mouse_x) * 2 + 1
  }`;
...
function get_param_values() {
  ...
  let values = {};
  for (let [key, imp] of Object.entries(IMPS)) values[key] = imp.encode_input();
  return values;
}
...
function display_grid(param_dict) {
...
function load_puzzle(puzzle) {
  if (!is_json(puzzle)) puzzle = JSON.parse(puzzle);

  set_unsolved();

  let params = puzzle.param_values;
  for (let [key, imp] of Object.entries(IMPS)) imp.set_value(params[key]);
...
function display_example_buttons() {
  if (EXAMPLES[pt] == undefined) get("examples_div").innerHTML = "";
  else {
    let examples_html = "";
...
function display_solutions(solution_str) {
  ...
  let cycle_solution_html = `Solution <span id='solution_num'></span> of ${
    ge + num_solutions
  }`;
  if (num_solutions > 1)
